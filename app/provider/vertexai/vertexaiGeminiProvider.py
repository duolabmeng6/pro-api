...
class vertexaiGeminiProvider:
    ...
    def setDebugSave(self, name="vertexai_gemini"):
        name = name.replace("/", "-")
        # 获取当前脚本所在的目录
    ...
    async def sendChatCompletions(self, request) -> AsyncGenerator[str, None]:
        ...
        logger.name = f"vertexaiGeminiProvider.{id}.request.model"
...
if __name__ == "__main__":
    async def main():
        ...
        interface.setDebugSave("weather-vertexai_gemini_" + provider['mapped_model'])
